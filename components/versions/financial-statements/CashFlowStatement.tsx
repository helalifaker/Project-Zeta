...
export function CashFlowStatement(props: CashFlowStatementProps): JSX.Element {
  ...
  const totals = useMemo(() => {
    return projection.reduce((acc, year) => ({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          30-year cash flow statement (2023-2052) showing operating, investing, and financing activities
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right" colSpan={4}>Operating Activities</TableHead>
                ...
                <TableHead className="text-right" colSpan={3}>Cash Position</TableHead>
              </TableRow>
            ...
            <TableBody>
              {projection.map((year, index) => {
                ...
                const depreciation = historical ? toDecimal(historical.cfDepreciation) : year.depreciation;
                ...
                const operatingCashFlow = historical ? toDecimal(historical.netCashFromOperatingActivities) : year.operatingCashFlow;
                const investingCashFlow = historical ? toDecimal(historical.netCashFromInvestingActivities) : year.investingCashFlow;
                const financingCashFlow = historical ? toDecimal(historical.netCashFromFinancingActivities) : year.financingCashFlow;
                const netCashFlow = historical ? toDecimal(historical.netIncreaseDecreaseCash) : year.netCashFlow;
                ...
                const theoreticalCash = isHistorical ? beginningCash.plus(netCashFlow) : year.theoreticalCash;
                ...
                return (
                  <TableRow key={year.year} className={isHistorical ? 'bg-blue-50/50 dark:bg-blue-950/20' : ''}>
                    ...
                    <TableCell className={`text-right font-mono text-sm ${workingCapitalChange.isNegative() ? 'text-accent-green' : 'text-accent-red'}`}>
                      {formatCurrency(workingCapitalChange)}
                    </TableCell>
                    <TableCell className={`text-right font-mono text-sm font-semibold ${isOperatingPositive ? 'text-accent-green' : 'text-accent-red'}`}>
                      {isOperatingPositive ? (
                    ...
                    <TableCell className={`text-right font-mono text-sm font-semibold ${hasDebtChange ? (financingCashFlow.greaterThan(0) ? 'text-accent-red' : 'text-accent-green') : ''}`}>
                      {hasDebtChange ? (
                        <div className="flex flex-col">
                          <span>{formatCurrency(financingCashFlow, true)}</span>
                          <Badge variant={financingCashFlow.greaterThan(0) ? 'destructive' : 'default'} className="text-xs mt-1">
                    ...
                    {/* Net Cash Flow */}
                    <TableCell className={`text-right font-mono text-sm font-bold ${isNetPositive ? 'text-accent-green' : 'text-accent-red'}`}>
              ...
              <TableRow className="bg-background-tertiary font-semibold border-t-2">
                ...
                <TableCell className={`text-right font-mono ${totals.workingCapitalChange.isNegative() ? 'text-accent-green' : 'text-accent-red'}`}>
                  {formatCurrency(totals.workingCapitalChange)}
                </TableCell>
                <TableCell className={`text-right font-mono ${totals.operatingCashFlow.greaterThan(0) ? 'text-accent-green' : 'text-accent-red'}`}>
                ...
                {/* Net */}
                <TableCell className={`text-right font-mono font-bold ${totals.netCashFlow.greaterThan(0) ? 'text-accent-green' : 'text-accent-red'}`}>
        ...
        <div className="mt-4 p-4 bg-background-tertiary rounded-md space-y-2">
          <p className="text-sm text-text-secondary">
            <span className="font-semibold">Formula:</span> Operating CF = Net Income + Depreciation - Working Capital Changes
          </p>
          <p className="text-sm text-text-secondary">
            <span className="font-semibold">Cash Reconciliation:</span> Theoretical Ending Cash = Beginning Cash + Net Cash Flow. 
            Actual Ending Cash is adjusted to maintain minimum balance (1M SAR), with debt created/repaid as needed.
          </p>
