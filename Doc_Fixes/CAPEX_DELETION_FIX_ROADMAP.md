...
**Priority:** 🟢 **LOW** (Simplification - mostly verification)
...
1. **Auto Re-investment Rule Behavior:**
   - ✅ Rule generates recurring items based on cycle years (e.g., every 7 years)
   - ✅ If user **deletes the RULE**, all auto-generated items should be deleted
   - ❌ **NO possibility to delete individual auto items** (simplification)

2. **Manual Items Behavior:**
   - ✅ Manual items are **ADDITIVE** (not overrides)
   - ✅ If auto item exists: Year 2035, FURNITURE, 5M SAR
   - ✅ And manual item exists: Year 2035, FURNITURE, 3M SAR
   - ✅ **Both should exist** (total = 8M SAR for that year/category)
   - ✅ No conflict resolution needed

3. **Current Problem:**
   - ❌ User might be able to delete individual auto items (should not be possible)
   - ❌ If auto items are deleted, they reappear when rule recalculates
   - ✅ **Solution: Prevent deletion of individual auto items entirely**
...
- ✅ **No schema changes needed** (no suppressed years tracking)
- ✅ **No complex logic** (no filtering during recalculation)
- ✅ **Clear user experience** (delete rule = delete all auto items)
- ✅ **Simple implementation** (mostly verification and enforcement)
...
5. If user deletes rule:
   ...
   - Manual items remain
   ↓
6. Result: Clear, simple behavior ✅
```
...
- ✅ Auto items should NOT show delete button (current behavior appears correct)
- ✅ Only manual items should show delete button
- ⚠️ **Action Required:** Verify this is working correctly
...
- ✅ API only deletes items where `ruleId IS NULL` (manual items)
- ✅ API only creates items with `ruleId: null` (manual items)
- ⚠️ **Action Required:** Verify that auto items (ruleId IS NOT NULL) cannot be deleted via API
...
- ✅ Auto items should NOT show delete button (current behavior appears correct)
- ✅ Only manual items should show delete button
- ⚠️ **Action Required:** Verify this is working correctly in UI
...
- ✅ Current logic already deletes all auto items when rule is deleted
- ✅ This is correct behavior (as per requirements)
- ✅ No changes needed
...
4. User understands: Cannot delete individual auto items
   ↓
5. Result: Clear UX, no confusion ✅
```
...
2. System:
   ...
   ↓
3. Result: All auto items deleted, manual items remain ✅
```
...
2. System recalculates:
   ...
   ↓
3. Result: All auto items regenerated (expected behavior) ✅
```
...
3. Both items exist in table:
   ...
   ↓
4. Total for 2035, FURNITURE: 8M SAR (additive) ✅
```
...
- ✅ Cascade delete removes all auto items (ruleId = rule.id)
- ✅ Manual items remain (ruleId = null)
- ✅ This is the only way to remove auto items
...
- ✅ Manual items are additive (correct)
- ✅ No conflict resolution needed (correct)
- ✅ Manual items can coexist with auto items (correct)
...
- [ ] Test: Attempt to delete auto item via UI → should not be possible
- [ ] Test: Attempt to delete auto item via API → should be ignored
- [ ] Test: Delete rule → verify all auto items deleted
- [ ] Test: Manual item + auto item same year → verify both exist (additive)
- [ ] Test: Update rule → verify auto items regenerate correctly
...
1. ✅ **Cannot Delete Auto Items**: User cannot delete individual auto items (UI + API protection)
2. ✅ **Delete Rule**: Deleting rule deletes all auto items (existing behavior - correct)
3. ✅ **Manual Items Additive**: Manual items coexist with auto items (existing behavior - correct)
4. ✅ **Clear UX**: User understands that auto items cannot be deleted individually

### Non-Functional Requirements

1. ✅ **Performance**: No performance impact (simplification, not addition)
2. ✅ **Data Integrity**: Auto items only deleted when rule is deleted
3. ✅ **User Experience**: Clear visual distinction between auto and manual items
...
- Manual items: 2030, TECHNOLOGY, 2M SAR (remains) ✅

**Result:** All auto items deleted, manual items remain ✅
...
- Total for 2035, FURNITURE: 8M SAR (additive) ✅

**Result:** Both items exist (additive) ✅
...
- Auto items: 2028, 2035, 2042, 2049 (regenerated with new amounts) ✅

**Result:** All auto items regenerated (expected behavior) ✅
...
**Expected:** ✅ No delete button on auto items
...
**Expected:** ✅ API rejects/ignores auto item deletion
...
**Expected:** ✅ Auto items deleted, manual items remain
...
**Expected:** ✅ Both items exist (additive), correct button visibility
...
**Expected:** ✅ All auto items regenerated (expected behavior)
...
1. ✅ **Auto Re-investment Rule**: Generates recurring items based on cycle
2. ✅ **Delete Rule**: Deletes all auto items (existing behavior - correct)
3. ❌ **NO individual auto item deletion**: Simplification - not possible
4. ✅ **Manual Items**: Additive (no override) - existing behavior correct
...
1. ✅ **Manual items are additive** (no changes needed)
2. ✅ **Delete rule deletes all auto items** (no changes needed)
3. ✅ **Auto items generation logic** (no changes needed)
...
**Risk Level:** 🟢 **LOW** (simplification, no schema changes)
