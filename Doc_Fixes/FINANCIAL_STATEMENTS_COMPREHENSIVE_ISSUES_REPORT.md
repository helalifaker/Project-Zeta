...
const expectedEBITDA = testRevenue.minus(testStaffCost).minus(testRent).minus(testOpex);
// Expected: -576,520 SAR (negative, but small)

const result = calculateEBITDAForYear(testRevenue, testStaffCost, testRent, testOpex);
...
console.log(
  'Historical Capex Items:',
  capexItems.filter((item) => {
    const itemYear = typeof item.year === 'number' ? item.year : parseInt(String(item.year), 10);
    return itemYear < startYear;
  })
);
...
const testDepreciationRate = new Decimal(0.1);
