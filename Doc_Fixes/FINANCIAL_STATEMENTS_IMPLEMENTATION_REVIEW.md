...
export async function calculateFullProjection(
  ...
  const startingCash = bsSettingsResult.success
    ? bsSettingsResult.data.startingCash
    : new Decimal(0);
  const openingEquity = bsSettingsResult.success
    ? bsSettingsResult.data.openingEquity
    : new Decimal(0);
  ...
  const solverParams: SolverParams = {
    ...
    depreciationRate: new Decimal(0.1), // Default 10%
