...
export async function getRent2024A(versionId: string): Promise<Result<Decimal>> {
  try {
...
for (let year = startYear; year <= endYear; year++) {
  ...
  if (period === 'TRANSITION') {
    ...
    if (historicalDataResult.success && historicalDataResult.data?.rent) {
      ...
      console.warn(
        `[HISTORICAL] Historical rent not available, using placeholder for year ${year}`
      );
      rentByYear.push({
        year,
        rent: new Decimal(0),
...
- CFO/Stakeholder: ********\_******** Date: **\_\_\_\_**
