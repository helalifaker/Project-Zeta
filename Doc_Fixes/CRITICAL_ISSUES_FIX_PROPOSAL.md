...
2. **Check Depreciation in Solver Result:**
   ...
   console.log(
     'Solver Result (2028):',
     result.data.projection.find((p) => p.year === 2028)
   );
   console.log(
     'Depreciation (2028):',
     result.data.projection.find((p) => p.year === 2028)?.depreciation
   );
   ```
...
if (result.success && result.data.converged) {
  ...
  console.log(
    '[DEBUG] Depreciation for Years with Capex:',
