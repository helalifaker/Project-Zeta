...
- ✅ Manual items should **override** auto-calculated items
- ✅ Users should be able to suppress auto items for specific years
- ❌ **MISSING:** Mechanism to suppress auto items (convert to manual override with 0)
...
3. User deletes item for 2035
   ↓
4. Database: Item deleted (ruleId = 'xxx', year = 2035) ❌ DELETED
   ↓
...
6. System recalculates:
   ...
   - Regenerates ALL items (2028, 2035, 2042, 2049) ❌ 2035 REAPPEARS
   ↓
7. Result: Deleted item (2035) is back ❌
```
...
6. System recalculates:
   ...
   ↓
7. Result: 2035 remains suppressed ✅
```
...
1. ✅ **Auto Items Generation**: Works correctly
2. ✅ **Manual Items Creation**: Works correctly
3. ❌ **Suppression Mechanism**: **MISSING**
4. ❌ **Override Logic**: **MISSING**
5. ❌ **Persistence of Deletions**: **MISSING**
...
const calcResult = await calculateAndPersistCapexItems(id, rulesForCalculation, cpiRate);
```
...
- ✅ Aligns with PRD requirement for "manual overrides"
- ✅ Simple implementation
- ✅ Clear user intent (manual item = override)

**Cons:**

- ⚠️ Shows "0" amount in table (could be confusing)
- ⚠️ Requires UI change to allow "suppress" action on auto items
...
- ✅ Clean separation (suppression is rule-level)
- ✅ No zero-amount items in table

**Cons:**

- ⚠️ Requires schema change (migration)
- ⚠️ More complex logic
- ⚠️ Not aligned with PRD "manual overrides" concept
...
- ⚠️ Requires schema change (migration)
- ⚠️ Suppressed items still in database (could be confusing)
- ⚠️ Not aligned with PRD "manual overrides" concept
...
1. ✅ **Aligns with PRD**: PRD explicitly mentions "manual overrides"
2. ✅ **No Schema Change**: Uses existing `ruleId = null` for manual items
3. ✅ **Clear User Intent**: Manual item = user override
