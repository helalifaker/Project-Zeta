...
function calculateTransitionRent(
  ...
  switch (config.model) {
    case 'FIXED_ESCALATION':
      ...
      return success(baseRent.times(Decimal.add(1, escalationRate).pow(yearsFromBase)));
...
export async function calculateFullProjection(
  ...
  for (let year = params.startYear; year <= params.endYear; year++) {
    ...
    const yearProjection = await calculateYearProjection(year, period, historicalData, params);
...
export const VALIDATION_PROFILES: Record<ValidationProfile, ValidationRules> = {
  ACCOUNTING_GRADE: {
    ...
    maxRevenueGrowthRate: 0.3,
    minRevenueGrowthRate: -0.3,
  ...
  PLANNING_GRADE: {
    ...
    enforceTypes: true,
    allowNegativeRevenue: false,
    maxRevenueGrowthRate: 0.5,
    minRevenueGrowthRate: -0.5,
    enforceRevenueCalculation: true,
  ...
  DRAFT_GRADE: {
    ...
    enforceTypes: true,
    allowNegativeRevenue: false,
    maxRevenueGrowthRate: 1.0, // 100% max
    minRevenueGrowthRate: -1.0, // 100% max decline
