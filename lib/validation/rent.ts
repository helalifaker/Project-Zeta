...
const FixedEscalationParamsSchema = z.object({
  baseRent: z.number().positive('Base rent must be positive').finite(),
...
const PartnerModelParamsSchema = z.object({
  landSize: z.number().positive('Land size must be positive').finite(),
  landPricePerSqm: z.number().positive('Land price must be positive').finite(),
  buaSize: z.number().positive('BUA size must be positive').finite(),
  constructionCostPerSqm: z.number().positive('Construction cost must be positive').finite(),
