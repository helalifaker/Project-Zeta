...
const WARNING_MESSAGES = {
  slow_convergence: "Calculations converged slowly. Results may be less accurate for scenarios with high debt.",
  not_converged: "⚠️ Calculations did not fully converge. Simplified assumptions were used. Review your assumptions or contact support.",
...
class PerformanceMonitor {
  static track(operation: string, duration: number, context: any) {
    ...
    if (duration > 200) {
      console.error(`🔴 ${operation} exceeded 200ms worst-case target: ${duration.toFixed(2)}ms`, context);
    }
  }
}
...
export async function PUT(
  ...
  const result = await updateOtherRevenueBulk(
    params.id,
