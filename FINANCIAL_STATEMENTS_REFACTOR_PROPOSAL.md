  ...
  try {
    ...
    for (const ebitdaItem of ebitdaByYear) {
      ...
      const operatingCashFlow = netIncome
        .plus(depreciation)
        .minus(workingCapitalChange); // WC increase uses cash (subtract), WC decrease provides cash (add)
...
export async function calculateFullProjection(
  ...
  const solverParams: SolverParams = {
    ...
    depreciationRate: new Decimal(0.10), // TODO: Get from admin settings or provide as param
