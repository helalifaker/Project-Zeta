...
export async function getRent2024A(
  ...
  try {
    ...
    console.warn(`[PLACEHOLDER] 2024A rent not available for version ${versionId}, using placeholder`);
...
if (historicalYears.length > 0) {
  const historicalResult = await getHistoricalDataBatch(versionId, historicalYears as (2023 | 2024)[]);
...
export async function POST(req: Request) {
  ...
  const validationTier = (formData.get('validationTier') as ValidationTier) || ValidationTier.STRICT;
