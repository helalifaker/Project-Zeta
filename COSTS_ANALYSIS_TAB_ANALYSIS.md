...
export function calculateRentForYear(model: RentModel, params: unknown, year: number): Result<{rent: number}>
export function calculateTotalRent(params: RentCalculationParams): Result<number>

// Specific models
export function calculateFixedEscalationRent(params: FixedEscalationParams): Result<FixedEscalationResult[]>
...
// Usage example:
const capexResult = calculateCapexFromRules(
  version.capexRules,
  adminSettings.cpiRate,
  2023,
  2052
);
...
const avgRentLoad = rentResult.data.reduce((sum, r) => sum + r.rentLoad, 0) / rentResult.data.length;
```
